...
====================
Modulo condiviso per il calcolo delle metriche di retrieval (Hit Rate, Precision, MRR).
Funziona in modo indipendente per il sotto-progetto general_pdf_pipeline.
"""
...
def _chunk_corretto(meta: Dict, expected_file: str, expected_page: str,
    ...
    # Rimuoviamo eventuali percorsi cartella se presenti in meta['file_name']
    found_file = meta.get('file_name', '')
    if '/' in found_file:
        found_file = found_file.split('/')[-1]
    if '\\' in found_file:
        found_file = found_file.split('\\')[-1]
        
    exp_file = expected_file
    if '/' in exp_file:
        exp_file = exp_file.split('/')[-1]
    if '\\' in exp_file:
        exp_file = exp_file.split('\\')[-1]

    if found_file != exp_file:
        return False
...
def stampa_report(db_name: str, metriche: Dict[str, Any]) -> None:
    """Stampa un report leggibile in console."""
    k   = metriche.get("k", 3)
