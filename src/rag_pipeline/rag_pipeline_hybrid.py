...
# Import per il retriever ibrido
...
def format_docs_with_sources(docs):
    ...
    print(f"\n--- 🔎 CHUNK RECUPERATI (IBRIDO: BM25 + Vector DB) ---")
    print(f"Numero totale di chunk recuperati post-reranking: {len(docs)}")
...
def main():
    parser = argparse.ArgumentParser(description="Testa la RAG Pipeline (Ibrido) dal terminale")
    ...
    print(f"🔄 Avvio test pipeline RAG IBRIDO in modalità: {args.env.upper()}")
    ...
    print("🗄️ Caricamento Vector DB e preparazione BM25...")
    ...
    # 2. Retriever sparso (Keyword Search / BM25)
    # Estraiamo tutti i documenti dal Chroma DB per inizializzare il BM25
    # NOTA: Per dataset molto grandi, in produzione si dovrebbe mantenere un indice BM25 separato persistito (es. Elasticsearch)
    all_data = db.get()
    
    # Se il DB è vuoto
    if not all_data or not all_data.get('documents'):
        print("❌ Nessun documento trovato nel database vettoriale.")
        return
        
    docs = []
    for doc, meta in zip(all_data['documents'], all_data['metadatas']):
        # Salta eventuali documenti senza testo
        if doc:
            docs.append(Document(page_content=doc, metadata=meta))
            
    print(f"✅ Inizializzazione BM25 su {len(docs)} chunk...")
    bm25_retriever = BM25Retriever.from_documents(docs)
    bm25_retriever.k = 3
    
    # 3. Retriever Ibrido (Ensemble)
    print("🤝 Creazione Retriever Ibrido (50% Dense, 50% Sparse)...")
    ensemble_retriever = EnsembleRetriever(
        retrievers=[bm25_retriever, chroma_retriever],
        weights=[0.5, 0.5] # Bilanciamento uguale
    )

    try:
        chain = setup_rag_chain(ensemble_retriever, env=args.env)
