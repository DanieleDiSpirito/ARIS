...
def is_garbage_line(line):
    ...
    """
    test_line = line.replace("[Caption]:", "").replace("<!-- image -->", "").strip()
    if len(test_line) < 3:
        return True
    
    # 1. Rileva didascalie di figure e tabelle
    if re.match(r'^(Fig\.|Figure|Table|Tab\.)\s*\d+', test_line, re.IGNORECASE):
        return True
        
    # 2. Rileva descrizioni visive orfane residue
...
def fix_hierarchy(record):
    """
    Regola la gerarchia della sezione in base al titolo estratto.
    """
...
def preprocess_knowledge_base(input_folder, output_file):
    """
    Carica i file estratti da MinerU, applica filtri di pulizia avanzati,
    risolve le cross-references tramite domain enrichment e salva il file finale JSON.
    """
    all_records = []
    json_files = glob.glob(os.path.join(input_folder, "*.json"))
    
    print(f"🧹 [MINERU CLEANER] Inizio Preprocessing di {len(json_files)} file da {input_folder}...")
    ...
    for file_path in json_files:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for rec in data:
                ...
                # Rimuove righe non informative, didascalie o rumore grafico
                ...
                # Se dopo la pulizia il testo è vuoto o sotto la soglia di 15 caratteri, lo scartiamo
    ...
    print(f"📂 [MINERU CLEANER] Knowledge Base finale salvata con successo in: {output_file} ({len(all_records)} record)\n")

if __name__ == "__main__":
    percorso_input = os.path.join(PROJECT_ROOT, "data", "processed", "mineru")
    percorso_output = os.path.join(PROJECT_ROOT, "data", "processed", "knowledge", "kb_mineru.json")
