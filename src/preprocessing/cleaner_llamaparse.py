...
def preprocess_knowledge_base(input_folder, output_file):
    """
    Carica i file estratti da LlamaParse, applica filtri di pulizia avanzati,
    risolve le cross-references tramite domain enrichment e salva il file finale JSON.
    """
    all_records = []
    json_files = glob.glob(os.path.join(input_folder, "*.json"))
    
    print(f"🧹 [LLAMAPARSE CLEANER] Inizio Preprocessing di {len(json_files)} file da {input_folder}...")
    ...
    print(f"📂 [LLAMAPARSE CLEANER] Knowledge Base finale salvata con successo in: {output_file} ({len(all_records)} record)\n")

if __name__ == "__main__":
    percorso_input = os.path.join(PROJECT_ROOT, "data", "processed", "llamaparse")
    percorso_output = os.path.join(PROJECT_ROOT, "data", "processed", "knowledge", "kb_llamaparse.json")
