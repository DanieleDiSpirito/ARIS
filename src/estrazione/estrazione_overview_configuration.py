...
def estrai_testo_manuale():
    ...
    try:
            ...
            for num_pagina, pagina_plumber in enumerate(pdf.pages):
                ...
                for tabella in tabelle_trovate:
                    ...
                    for riga in dati_tabella[1:]:
                        ...
                        testo_strutturato.append(f"Specifiche per {chiave_principale} -> " + " | ".join(frase_dati))
        ...
        with fitz.open(percorso_pdf) as doc:
            for num_pagina in range(len(doc)):
                ...
                for blocco in blocchi:
                    ...
                    # 3. FILTRO ANTIRUMORE PER IMMAGINI (Salva Captions, butta Etichette)
                    numero_parole = len(testo_pulito.split())
                    if numero_parole < 5 and not testo_pulito.lower().startswith(("fig", "table", "note", "warning")):
                        continue

                    # 4. CONTROLLO TITOLI E SEZIONI
        ...
        print(f"Magia completata! Ho estratto {len(dati_estratti)} blocchi strutturati nel file: {percorso_output}")
