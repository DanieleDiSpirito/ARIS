...
valuta_rag_graph.py
===================
Valutazione del retriever con Grafo (BM25 + Vector + Graph Expansion) con metriche complete:
...
from rag_pipeline_graph import build_graph_retriever
...
def valuta_db(db_path: str, env: str, test_file: str, k: int = 4,
              tolleranza: int = 1, debug: bool = False) -> dict:
    """
    Retrieval con Grafo (BM25 + Chroma + Graph Expansion) con calcolo completo delle metriche.
    """
    ...
    # Inizializziamo il GraphRAG Retriever (base k_base = k * 2, top_n=k)
    k_base = max(k * 2, 8)
    retriever_graph = build_graph_retriever(lc_chroma, k_base=k_base, top_n=k)
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc=f"Valutazione {os.path.basename(db_path)}"):
        ...
        results = retriever_graph.invoke(row['question'])[:k]
        ...
        if debug:
            ...
            if not trovato:
                ...
                print(f"   Top {k} trovati dopo GraphRAG (tolleranza ±{tolleranza}):")
...
def main():
    parser = argparse.ArgumentParser(
        description="Valuta il RAG con Grafo (BM25 + Vettoriale + Graph Expansion)",
    ...
    print(
        f"🔄 Avvio valutazione GraphRAG — env={args.env} "
    ...
    if summary_results:
        ...
        print("\n📊 TABELLA COMPARATIVA DI RETRIEVAL GRAPHRAG:")
        print("=" * 90)
        ...
        report_path = os.path.join(metrics_dir, "benchmark_retrieval_graph.md")
        
        md_table = df_summary.to_markdown(index=False)
        report_content = f"""# Benchmark Retrieval GraphRAG (ARIS)
