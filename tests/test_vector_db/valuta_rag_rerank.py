"""
valuta_rag_rerank.py
====================
Valutazione del retriever con Re-ranking (BM25 + Vector + Cross-Encoder) con metriche complete:
...
# Aggiungi la cartella della pipeline al path per importare il Reranker
sys.path.append(os.path.join(PROJECT_ROOT, "src", "rag_pipeline"))
from rag_pipeline_rerank import HybridRerankRetriever
...
def valuta_db(db_path: str, env: str, test_file: str, k: int = 3,
    ...
    """
    Retrieval con Re-ranking (BM25 + Chroma + Cross-Encoder) con calcolo completo delle metriche.
    """
    ...
    # Impostiamo il numero di candidati grezzi da recuperare (k * 2) per poi filtrarli
    k_grezzo = max(k * 2, 6)
    ...
    retriever_vettoriale = lc_chroma.as_retriever(search_kwargs={"k": k_grezzo})
    ...
    if not os.path.exists(json_path):
        ...
        retriever_base = retriever_vettoriale
    else:
        docs_bm25 = carica_documenti_per_bm25(json_path)
        retriever_bm25 = BM25Retriever.from_documents(docs_bm25)
        retriever_bm25.k = k_grezzo
        retriever_base = EnsembleRetriever(
            retrievers=[retriever_bm25, retriever_vettoriale],
            weights=[0.5, 0.5]
        )

    # --- Reranker Retriever ---
    retriever_rerank = HybridRerankRetriever(retriever_base, top_n=k)
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc=f"Valutazione {os.path.basename(db_path)}"):
        ...
        results = retriever_rerank.invoke(row['question'])[:k]
        ...
        if debug:
            ...
            if not trovato:
                ...
                print(f"   Top {k} trovati dopo Re-ranking (tolleranza ±{tolleranza}):")
...
def main():
    parser = argparse.ArgumentParser(
        description="Valuta il RAG con Re-ranking (BM25 + Vettoriale + Cross-Encoder)",
    ...
    print(
        f"🔄 Avvio valutazione RAG con Re-ranking — env={args.env} "
    ...
    if summary_results:
        ...
        print("\n📊 TABELLA COMPARATIVA DI RETRIEVAL CON RE-RANKING:")
        print("=" * 90)
        ...
        report_path = os.path.join(metrics_dir, "benchmark_retrieval_rerank.md")
        
        md_table = df_summary.to_markdown(index=False)
        report_content = f"""# Benchmark Retrieval con Re-ranking (ARIS)
